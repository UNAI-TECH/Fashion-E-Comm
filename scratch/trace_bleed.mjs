...
// Trace back from an accidentally marked phone pixel (e.g. x=910, y=300)
const targetIdx = 300 * w + 910;
if (visited[targetIdx] === 1) {
  console.log("Tracing path to phone pixel (910, 300):");
  let curr = targetIdx;
  const path = [];
  while (curr !== -1) {
    const cx = curr % w;
    ...
    const row = pixels[cy];
    const r = row[cx * bpp];
    const g = row[cx * bpp + 1];
    const b = row[cx * bpp + 2];
    path.push({ x: cx, y: cy, rgb: `RGB(${r},${g},${b})` });
    curr = parent[curr];
  }
  // Print last 20 steps of the path (closest to the start border) and first 20 steps (closest to target)
  console.log(`Path length: ${path.length} steps`);
  console.log("Sampling path from border to target:");
  for (let i = path.length - 1; i >= 0; i -= 20) {
    const p = path[i];
    console.log(`  Step ${path.length - 1 - i}: (${p.x}, ${p.y}): ${p.rgb}`);
  }
  // Also print the final 5 steps
  console.log("Final steps:");
  path.slice(0, 5).reverse().forEach((p, idx) => {
    console.log(`  Step ${path.length - 5 + idx}: (${p.x}, ${p.y}): ${p.rgb}`);
  });
} else {
  console.log("Target pixel was not visited.");
}
