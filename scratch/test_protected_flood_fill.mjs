...
// Protected zone for the phone screen
...
for (let x = 0; x < w; x++) {
  let idx = 0 * w + x;
  let row = pixels[0];
  if (!isProtectedZone(x, 0) && isBackgroundColor(row[x * bpp], row[x * bpp + 1], row[x * bpp + 2])) {
    visited[idx] = 1;
    queue.push(idx);
  }
  idx = (h - 1) * w + x;
  row = pixels[h - 1];
  if (!isProtectedZone(x, h - 1) && isBackgroundColor(row[x * bpp], row[x * bpp + 1], row[x * bpp + 2])) {
    visited[idx] = 1;
    queue.push(idx);
  }
}
...
for (let y = 1; y < h - 1; y++) {
  ...
  idx = y * w + (w - 1);
  if (!isProtectedZone(w - 1, y) && isBackgroundColor(row[(w - 1) * bpp], row[(w - 1) * bpp + 1], row[(w - 1) * bpp + 2])) {
    visited[idx] = 1;
    queue.push(idx);
  }
}
...
console.log(`Protected Flood Fill results:`);
...
// Test if phone screen pixels were marked
let phoneMarked = 0;
