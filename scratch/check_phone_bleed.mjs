...
console.log("Printing coordinates of marked pixels inside phone screen box (x: 880-920, y: 200-300):");
for (let y = 200; y <= 300; y += 10) {
  let line = `y=${y}: `;
  for (let x = 880; x <= 920; x += 5) {
    const idx = y * w + x;
    line += `x=${x}:${visited[idx]} `;
  }
  console.log(line);
}
